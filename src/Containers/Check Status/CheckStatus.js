...
import { DenseAppBar, Paper, InputPage, DropdownPage } from "../../Components";
...
class CheckStatusUi extends React.Component {
  ...
  checkStatus = async () => {
    let { Month, RollNum, alldata, late, present } = this.state;
    present = 0;
    late = 0;
    this.setState({
      present: 0,
    ...
    if (RollNum === undefined) {
      ...
      await firebaseApp
        .firestore()
        .collection(`${this.state.className}`)
        .doc(`${RollNum}`)
        .get()
        .then((res) => {
            ...
            for (var key in data) {
              ...
              if (data[key].month === Month) {
                ...
                switch (data[key].attendance) {
                  case "present":
                    this.setState({ present: present + 1 });
                    break;
                  case "late":
                    this.setState({ late: late + 1 });
                    break;
                  case "":
                    alert("sorry");
                    break;
                  default:
                    this.setState({ late: 0, present: present + 0 });
                }
              } else {
                this.setState({
            ...
            <Paper>
              <div style={{ marginBottom: "15px" }}>
                <DropdownPage
                  ...
                  onChange={(e) =>
                    this.setState({ className: e.target.value, sec: false })
                  }
              ...
              <div style={{ marginBottom: "15px" }}>
                <DropdownPage
                  ...
                  disabled={this.state.sec}
                  onChange={(e) =>
                    this.setState({ section: e.target.value, roll: false })
                  }
                  list={this.state.classesSections}
              ...
              <InputPage
                ...
                label="Roll Number"
                onChange={(e) =>
                  this.setState({ RollNum: e.target.value, mont: false })
                }
              />
              <div style={{ marginBottom: "25px" }}>
                <DropdownPage
                  value={this.state.Month}
                  disabled={this.state.mont}
                  onChange={(e) =>
                    this.setState({ Month: e.target.value, disBtn: false })
                  }
                  list={[
                    "Janurary",
                    "Feburary",
